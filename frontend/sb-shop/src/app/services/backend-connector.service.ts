...
export class BackendConnectorService {
  ...
  updateEpisode(
    item: spongebobepisodesmodel
  ): Observable<spongebobepisodesmodel> {
    let bodyEncoded = new URLSearchParams();
    bodyEncoded.append('id', item.idspongebobepisodes.toString());
    bodyEncoded.append('image', item.image);
    bodyEncoded.append('name', item.name);
    bodyEncoded.append('liked', item.liked);
    const body = bodyEncoded.toString();
    return this.httpClient.put<spongebobepisodesmodel>(
      this.endpointepisodes + '/' + item.idspongebobepisodes,
      body,
      httpOptionsUsingUrlEncoded
    );
  }
  ...
  getAllEpisodes(): Observable<spongebobepisodesmodel[]> {
    return this.httpClient.get<spongebobepisodesmodel[]>(this.endpointepisodes);
  }
}
